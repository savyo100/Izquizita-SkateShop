    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-dark-900">
      <aside className="bg-dark-800 border-b border-gray-700 md:border-b-0 md:border-r md:w-64 p-4 md:p-6 flex flex-col">
        ...
        <div className={`${mostrarFiltroMobile ? "block" : "hidden"} md:block mt-4`}>
          <label htmlFor="filtro-tipo" className="text-foreground font-semibold mb-2 block">
            Tipo de produto
          </label>
          <select
            id="filtro-tipo"
            ...
            className="w-full rounded-md border border-gray-600 bg-dark-700 py-2 px-3 text-foreground placeholder-gray-400 focus:border-neon-green focus:outline-none focus:ring-2 focus:ring-neon-green transition"
          >
            {tiposProduto.map(t => (
      ...
      <main className="flex-1 p-10 overflow-auto">
        {produtosFiltrados.length > 0 ? (
          ...
          <p className="text-foreground">Nenhum produto encontrado para o filtro selecionado.</p>
        )}
      </main>
    </div>
  );
