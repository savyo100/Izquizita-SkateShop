...
// Funções do Firebase Firestore para pegar dados de uma coleção
import { collection, getDocs } from "firebase/firestore";
import { db } from "../firebase"; // instância do Firestore configurada

// Componentes e tipos do projeto
...
// Definindo os tipos de produtos possíveis (const assertion garante tipo literal)
const tiposProduto = ["todos", "shape", "rodas", "truck", "parafusos", "acessorios", "roupas", "lixa"] as const;

export default function Produtos() {
  // Estado dos produtos carregados do Firestore
  ...
  // Estado para pesquisa por nome
  const [pesquisa, setPesquisa] = useState('');

  // Estado para controlar a exibição do filtro em mobile
  const [mostrarFiltroMobile, setMostrarFiltroMobile] = useState(false);

  // useEffect para carregar os produtos uma vez ao montar o componente
  useEffect(() => {
    const carregarProdutos = async () => {
      try {
        // Pega todos os documentos da coleção "produtos"
        const querySnapshot = await getDocs(collection(db, "produtos"));

        // Mapeia os documentos para o formato Produto, tratando valores inválidos
        ...
        setProdutos(lista); // salva produtos no estado
      } catch (err) {
  ...
  // Aplica filtros de tipo e pesquisa
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-background">
      ...
      <aside className="bg-card border-b border-border lg:border-b-0 lg:border-r lg:w-80 p-6 flex flex-col shadow-lg">
        ...
        {/* Conteúdo do filtro (aparece em mobile quando toggled) */}
