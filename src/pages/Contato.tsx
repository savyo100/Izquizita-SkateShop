...
const Contato = () => {
  ...
  // Função para abrir WhatsApp em nova aba com mensagem pré-definida
  const handleWhatsAppClick = () => {
    const url = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(whatsappMessage)}`;
    window.open(url, '_blank');
  };

  // Estado para controlar feedback de "copiado"
  const [copiado, setCopiado] = useState<string | null>(null);

  // Função para copiar texto para clipboard
  // Verifica se navigator.clipboard.writeText está disponível
  // Caso contrário, usa fallback com input temporário
  const copiarTexto = (texto: string, label: string) => {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      // Se suportado, copia diretamente
      navigator.clipboard.writeText(texto)
        .then(() => setCopiado(label)) // Mostra feedback de cópia
        .catch(err => console.error("Erro ao copiar:", err));
    } else {
      // Fallback para navegadores antigos ou contextos sem clipboard API
    ...
    // Remove o feedback de "copiado" após 2 segundos
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-dark-800 rounded-lg p-8 border border-gray-700 hover:border-neon-green transition-colors duration-300">
            <div className="text-center">
              ...
              <h3 className="text-xl font-bold text-foreground mb-3">Visite Nossa Loja</h3>
              <p className="text-black dark:text-gray-400 mb-6">
                Venha conhecer nossa loja física! Experimente os produtos,
                converse com nossa equipe e faça parte da comunidade Izquizita.
              </p>
              <div className="text-black dark:text-gray-400 mb-6">
          ...
          <h2 className="text-2xl font-bold text-neon-green mb-6 text-center">
            ...
            <div className="p-4">
              ...
              <p className="text-black dark:text-gray-400 text-sm">
                contato@Izquizita.com.br
              </p>
              <button
                onClick={() => copiarTexto("contato@Izquizita.com.br", "E-mail")}
                className="mt-2 text-xs text-neon-green hover:underline"
              >
                Copiar E-mail
              </button>
              {copiado === "E-mail" && <p className="text-green-400 text-xs mt-1">Copiado!</p>}
            </div>
            ...
            <div className="p-4">
              ...
              <button
                onClick={() => copiarTexto("@Izquizita_oficial", "Instagram")}
                className="mt-2 text-xs text-neon-green hover:underline"
              >
                Copiar Instagram
              </button>
              {copiado === "Instagram" && <p className="text-green-400 text-xs mt-1">Copiado!</p>}
            </div>
            ...
            <div className="p-4">
              ...
              <p className="text-black dark:text-gray-400 text-sm">
                Avenida Tomaz Rebelo, 123 - Centro
              </p>
              <button
                onClick={() =>
                  copiarTexto("Avenida Tomaz Rebelo, 123 - Centro", "Endereço")
                }
                className="mt-2 text-xs text-neon-green hover:underline"
              >
                Copiar Endereço
              </button>
              {copiado === "Endereço" && <p className="text-green-400 text-xs mt-1">Copiado!</p>}
            </div>
          </div>
        </div>
